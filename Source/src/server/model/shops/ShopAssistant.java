...
public class ShopAssistant {
	...
	public void handleOtherShop(int itemID) {
		if (c.myShopId == 20) {
			if (c.pcPoints >= getSpecialItemValue(itemID)) {
				...
				c.sendMessage("You do not have enough FXP points to buy this item.");
			}
			...
			if (c.mbPoints >= getSpecialItemValue(itemID)) {
				...
				c.sendMessage("You do not have enough Mage Arena Points to buy this item.");
			}
	...
	public void openSkillCape() {
		...
		/*
		 * if (capes > 1) capes = 1; else capes = 0;
		 */
		c.myShopId = 15;
