...
public class TradeAndDuel{
	...
	**/
	...
	public void openTrade() {
		...
		if(c.getItems().playerHasItem(15015, 1)) {
		c.getItems().deleteItem(15015,c.getItems().getItemSlot(15015),999999999);
		}
		if(c.getItems().playerHasItem(15016, 1)) {
...
{
	c.sendMessage("Gabbes Anti - Dupe: You don't have that amount!");
	return false;
}
		Client o = (Client) Server.playerHandler.players[c.tradeWith];
		if(o == null) {
...
if(!((c.playerItems[fromSlot] == itemID+1) && (c.playerItemsN[fromSlot] >= amount)))
{
	...
	public void startDuel() {
		...
		if(c.duelRule[7]){
			...
			for(int p = 0; p < c.CURSE.length; p++) { // reset curse glows 
