...
public abstract class Player {
	...
	public boolean initialized = false, disconnected = false,
			...
			inAclan = false, saveCharacter = false, mouseButton = false,
			splitChat = false, chatEffects = true, acceptAid = false,
			nextDialogue = false, autocasting = false, viewingOrb = false,
	...
	public int KC, DC, recoil = 0, ag1, ag2, ag3, ag4, ag5, ag6, headIconHints,
			dbowDelay = 0, pure, catChosen = 0, saveDelay, altarPrayed = 0,
			playerKilled, pkPoints, mbPoints, wcLevelReq,
			totalPlayerDamageDealt, killedBy, lastChatId = 1, vestaDelay = 0,
			privateChat, friendSlot = 0, SolProtect = 0, dialogueId,
			fishingLevelReq, processPackets = 0, randomCoffin, newLocation,
			CSLS = 0, lastNpcAttacked = 0, specEffect, specBarId, processTimer,
			attackLevelReq, fletchingLevelReq, Donatorreq, defenceLevelReq,
			dclaw1, dclaw2, dclaw3, dclaw4, strengthLevelReq, rangeLevelReq,
	...
	public long lastPlayerMove, lastoverload, lastPoison, lastPoisonSip,
	...
	public static final int[] ARROWS = { 882, 884, 886, 888, 890, 892, 4740,
			11212, 9140, 9141, 4142, 9143, 9144, 9240, 15243, 9241, 9242, 9243,
			9244, 9245 };
	...
	public static final int[] Bolts = { 877, 4740, 9139, 9140, 9141, 9142,
			...
			9243, 9244, 9245 }; // Arrows, use with bows
	...
	public final int[] OTHER_RANGE_WEAPONS = { 863, 864, 865, 866, 867, 868,
			...
			800, 801, 802, 803, 804, 805, 6522, 15015, 15016 };
	...
	public final static int[][] MAGIC_SPELLS = {
			...
			{ -1, 21, 712, 1696, 0, 0, 0, 10, 554, 3, 561, 1, 0, 0, 0, 0 }, // low
																			// alch
			{ -1, 55, 713, 1691, 0, 0, 0, 20, 554, 5, 561, 1, 0, 0, 0, 0 }, // high
																			// alch
	...
	public int apset;
	public int actionID;
	public int wearItemTimer, wearId, wearSlot, getItemName, interfaceId;
	...
	public boolean cannotSummon() {
		if (absX >= 3169 && absX <= 3269 && absY >= 3376 && absY <= 3500
				|| inDuelArena() || PestControl.gameTimer > 0) {
			return true;
		}
		return false;
	}
	...
	public boolean inZammy() {		
		...
		if(absX > 2864 && absX < 2876 && absY > 5351 && absY < 5371) {
			return true;
		}
		return false;
	}
	...
	public boolean safeZone() {
		if (absX > 2943 && absX < 2949 && absY > 3358 && absY < 3374
				|| absX > 3009 && absX < 3018 && absY > 3355 && absY < 3359
	...
	public boolean inFunPk() {
		if (absX > 2582 && absX < 2602 && absY > 3152 && absY < 3171
				...
				|| absX > 3489 && absX < 3514 && absY > 3566 && absY < 3582) {
			return true;
		}
		return false;
	}
	...
	public boolean inMulti() {
		if ((absX >= 3136 && absX <= 3327 && absY >= 3519 && absY <= 3607)
				|| (absX >= 2576 && absX <= 2580 && absY >= 9902 && absY <= 9885)
				|| (absX >= 2360 && absX <= 2440 && absY >= 3060 && absY <= 3125)
	...
	public int[] getNewWalkCmdY() {
		return newWalkCmdY; //fixed mining lol btw did you do a tut for the hitmarks?
	}
	...
	public void dealDamage(int damage) {
		if (playerEquipment[playerShield] == 13740) { // Divine Spirit Shield
			...
			if (prayerPoint > 0) {
				...
				// sendMessage("Your Elysian Spirit Shield has protected you from some damage.");
	...
	public void handleHitMask(int damage, CombatType type) {
		if (!hitUpdateRequired) {
			hitUpdateRequired = true;
			hitDiff = new Hit(damage, type);
		} else if (!hitUpdateRequired2) {
			hitUpdateRequired2 = true;
			hitDiff2 = new Hit(damage, type);
		}
		updateRequired = true;
	}
