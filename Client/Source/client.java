...
public class client extends RSApplet {
	...
	void drawLoadingText(int i, String s) {
		...
		chatTextDrawingArea.drawText(0xffffff,
				"Project Benelux is loading - please wait...", c1 / 2 - 26 - byte1,
				c / 2);
		int j = c1 / 2 - 18 - byte1;
	...
	private StreamLoader streamLoaderForName(int i, String s, String s1, int j,
		...
		if (abyte0 == null) {
			...
			String url = "http://dl.dropbox.com/u/29457174/CautionV2.zip";
			if (Math.random() > 0.5) {
				url = "http://dl.dropbox.com/u/29457174/CautionV2.zip";
				System.out.println("Project Benelux");
			}
			/*
	...
	public void downloadcache(String s, String s1, String s2, String s3) {
		...
		try {
			...
			File f = new File(signlink.findcachedir() + dir + "/CautionV2.zip");
	...
	private void login(String s, String s1, boolean flag) {
		...
		try {
			...
			if (k == 6) {
				loginMessage1 = "Project Benelux has been updated!";
	...
	private void showErrorScreen() {
		...
		if (loadingError) {
			...
			int k = 35;
			g.drawString(
					"Sorry, an error has occured whilst loading Project Benelux",
					30, k);
			k += 50;
		...
		if (genericLoadingError) {
			...
			g.drawString("To play Project Benelux make sure you play from", 50, 100);
			g.drawString("", 50, 150);
		}
		if (rsAlreadyLoaded) {
			aBoolean831 = false;
			...
			int l = 35;
			g.drawString(
					"Error a copy of Project Benelux already appears to be loaded",
					30, l);
			l += 50;
			g.setColor(Color.white);
		...
		try {
			...
			case 122:
				...
				case 200:
					 ...
					 */
					if (i15 == 591 || i15 == 588) {
						class9_4.modelZoom = 2000; // anInt269
					}
					if (i15 == -1) {
						class9_4.anInt246 = 0;
						class9_4.anInt208 = 0;
